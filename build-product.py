...
# 替换Properties文件中的占位符为实际路径
def getCompletePropertiesPath(keyList, valueList):
...
# 计算 .zip 文件的MD5值
def getMD5(MD5FilePath):

    if os.path.isfile(MD5FilePath):
        fp = open(MD5FilePath, 'rb')
        contents = fp.read()
        fp.close()
        print("MD5 value is：", hashlib.md5(contents).hexdigest())
...
# 创建md5文件并写入md5值
def md5_create(name, msg, desktop_path):
    full_path = desktop_path + '/' + name + '.MD5'
...
if __name__ == "__main__":
    ...
    keyList = []
    valueList = []
    dic, dic2 = getCompletePropertiesPath(keyList, valueList)
    ...
    # 生成MD5文件
    MD5FilePath = curPath + '/release/' + zipFileName + '.zip'
    MD5_value = getMD5(MD5FilePath)
    desktop_path = curPath + '/release'
    md5_create(zipFileName + '.zip', MD5_value, desktop_path)
    
    res = readPropertiesValue(propertiesPath, 'bigscreen-ui.networksecurity.url')
